    ...
    urbanList.innerHTML =
      list.map(item => `
        <div class="product-card urban-item" data-id="${item.id}" style="cursor:pointer;">
          ...
          <div style="color:#e01c1c;font-size:0.95rem;margin-bottom:0.2rem;">공포 난이도: <span class="level-stars">${renderLevelStars(item.level)}</span></div>
        </div>
      `).join('');
...
function renderUrbanDetail(id) {
  ...
  urbanList.innerHTML = `
    <div class="product-card urban-item urban-detail" style="width:100%;max-width:600px;margin:0 auto;">
      ...
      <div style="color:#e01c1c;font-size:1rem;margin-bottom:0.8rem;">공포 난이도: <span class="level-stars">${renderLevelStars(data.level)}</span></div>
      <div class="urban-item-body" style="margin-top:1.2rem; font-size:1.1rem; line-height:1.7;">${data.detail || data.body}</div>
